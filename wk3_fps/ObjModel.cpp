...
ObjModel::ObjModel(const std::string &fileName)
{
	...
	for (auto& g : groups)
	{
		...
		MaterialInfo* material = materials[g->materialIndex];
		
		Texture* t = material->texture;
		if(!t) t = new Texture("models/empty.png");
		//if (t) t->bind();
		std::vector<tigl::Vertex> list;
		for (auto& f : g->faces) {
			
			for (auto& v : f.vertices) {
				//				tigl::addVertex(tigl::Vertex::PTN(vertices.at(v.position), texcoords.at(v.texcoord), normals.at(v.normal)));
				//tigl::Vertex v = tigl::Vertex::PT(vertices.at(v.position), texcoords.at(v.texcoord));
				tigl::Vertex vertex;
				vertex.position = vertices.at(v.position);
				vertex.texcoord = texcoords.at(v.texcoord);
				vertex.color = glm::vec4(material->diffuseColor, 1.0f);
				if (normals.empty())
					vertex.normal = { 0,1,0 };
				else
					vertex.normal = normals.at(v.normal);
				
				list.push_back(vertex);
			}

		}
		drawMap[material] = list;
		//tigl::end();
...
void ObjModel::draw()
{
	...
	std::for_each(drawMap.begin(), drawMap.end(), [](const std::pair<MaterialInfo*, std::vector<tigl::Vertex>>& mapPair)
		{	
			
			if (mapPair.first && mapPair.first->texture) {
				
				if (mapPair.first->texture->name == "models/empty.png") {
					tigl::shader->enableTexture(false);
				}
				else {
					tigl::shader->enableTexture(true);
					mapPair.first->texture->bind();
				}
			}
			
			tigl::drawVertices(GL_TRIANGLES, mapPair.second);
...
void ObjModel::loadMaterialFile( const std::string &fileName, const std::string &dirName )
{
	...
	while(!pFile.eof())
	{
		...
		{//TODO, diffuse color
			currentMaterial->diffuseColor = glm::vec3(std::stof(params[1]), std::stof(params[2]), std::stof(params[3]));
		}
		else if (params[0] == "ka")
		{//TODO, ambient color
		}
